...
token = '7487465375:AAHbRBLCsgJyX5eA9XD9F-nYV0p0Com9cGo'
bot = Bot(token=token)
dp = Dispatcher()

creator = 7561325825  # TODO: in constant class
prompts_channel = -1002293126655
...
def sets_msg(id):
    ...
    pictures_in_chat = types.InlineKeyboardButton(text='Генерация картинок в диалоге:', callback_data='pictures_in_dialog')
...
async def generate_response(*args, **kwargs):  # TODO: work
    return
...
async def start(message: Message):
    if is_banned(message.from_user.id):
        ...
        if str(message.from_user.id) not in settings:
            settings[str(message.from_user.id)] = {'reset': True, 'pictures_in_dialog': True, 'pictures_count': 5, 'imageai': 'sd'}
...
async def message_to_admin(message: Message, state: FSMContext):
    if is_banned(message.from_user.id):
        ...
        if message.text.lower() == 'отмена':
            ...
            await state.clear()
            await message.reply('Сообщение отправлено.\nОбсуждение бота в группе @neuro1_bot_community')
            await bot.send_message(creator,
                                   f'Сообщение от пользователя @{message.from_user.username}, {message.from_user.id}:')
            await message.forward(creator)
    ...
    if is_banned(message.from_user.id):
        ...
        if settings[str(message.from_user.id)]['pictures_in_dialog']:
            for prompt in response[0]:
                request = requests.post(f'https://https://api.r00t.us.kg/v1/image/{settings[str(message.from_user.id)]["imageai"]}', json={"prompt": prompt})
